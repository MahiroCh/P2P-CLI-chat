...
use error::Result;
use p2p_chat::{socket, pid, paths, logger};
...
impl Drop for DaemonCleanupGuard {
	fn drop(&mut self) {
		if let Err(err) = pid::cleanup(&paths::daemon_pidfile()) {
			...
			log::debug!("Daemon PID file cleanup error details: {err:?}");
		}

		if let Err(err) = socket::cleanup(&paths::daemon_socket()) {
			log::warn!("Failed to clean up daemon socket: {err}");
			log::debug!("Daemon socket cleanup error details: {err:?}");
		}
	}
}
...
async fn handle_daemon_init() -> Result<()> {
	...
	pid::create(&paths::daemon_pidfile(), &pid::this_proc_pid())
		.inspect_err(|err| {
			log::debug!("pid::create() in daemon::run() failed to create daemon PID file: {err:?}");
		})
		.map_err(|err| Error::new(ErrorKind::PidFileCreationFailed, err))?;

	let conn_listener = socket::create(&paths::daemon_socket())
		.inspect_err(|err| {
			log::debug!("socket::create() in daemon::run() failed to create daemon socket: {err:?}");
		})
	...
	crate::daemon::logic(&conn_listener, &mut sigterm).await
		.inspect_err(|err| {
			log::debug!(
		...
		tokio::select! {
			...
			accepted = conn_listener.accept() => {
				let (mut stream, _) = accepted.map_err(|err| {
					...
					Error::new(ErrorKind::SocketAcceptFailed, err)
				})?;
