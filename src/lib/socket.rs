...
pub fn create(path: &Path) -> Result<TokioUnixListener> {
	...
	fs::create_dir_all(parent).map_err(|source| Error::new(
		ErrorKind::CreateParentDir,
		source,
	))?;
	let listener = TokioUnixListener::bind(path)
		.map_err(|source| Error::new(
			ErrorKind::BindListener,
			source,
	))?;
